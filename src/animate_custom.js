...
export default class AnimateCCC extends React.Component {
  ...
  static propTypes = {
    //Props passed to the AnimateCCC component
  ...
  constructor() {
    ...
    this.state = {
      // properties passed from the lib in the animation.js file
      properties: {},
      error: false,
    };
  }
  ...
  //loads the libraries for create js and adobeAn
  loadLibs = () => {
    ({ AdobeAn } = window);
  ...
  initAdobeAn = () => {
    ...
    try {
      ...
      if (e.message === "Cannot read property 'getLibrary' of undefined") {
        const err = new Error(
          `Animation with name ${animationName} was not found`,
          'test'
        );
        err.name = 'AnimateCC';
        throw err;
    ...
    if (
      lib.properties.manifest.filter(({ type }) => type === 'image').length ===
      0
    ) {
  ...
  resizeCanvas = () => {
    ...
    if (dim.isResp) {
      if (
        (dim.respDim === 'width' && dim.lastW === iw) ||
        (dim.respDim === 'height' && dim.lastH === ih)
      ) {
        sRatio = this.lastS;
      } else if (!dim.isScale) {
        if (iw < w || ih < h) {
  ...
  render() {
    ...
    const color = AnimateCCC.hexToRgba(properties.color, properties.opacity);
    return (
      <div>
        <div
          ref={(el) => {
